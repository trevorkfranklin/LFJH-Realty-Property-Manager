                ...
                <>
                  <tr key={e.id} className="hover:bg-navy-700/40 transition-colors">
                    <td className="pl-5 py-3">
                      {hasTx && (
                        <button onClick={() => toggleExpanded(e.id)} className="text-slate-500 hover:text-slate-300">
                    ...
                    <td className="px-5 py-3 text-right">
                      {e.annualAmount > 0
                        ? <span className={e.balance <= 0 ? 'text-emerald-400' : 'text-yellow-400 font-semibold'}>{e.balance <= 0 ? '—' : fmt(e.balance)}</span>
                    ...
                    <td className="px-5 py-3">
                      {canEdit && (
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => openEdit(e)} className="text-slate-400 hover:text-white"><Pencil size={14} /></button>
                          {!e.derived && <button onClick={() => remove(e.id)} className="text-slate-400 hover:text-red-400"><Trash2 size={14} /></button>}
                        </div>
                      )}
                    </td>
                  </tr>
