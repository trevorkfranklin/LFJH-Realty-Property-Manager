// Legacy auto-assigned categories that were never intentionally chosen
...
export function buildPatternMap(transactions) {
  // Count votes for each (normalizedDescription → category + propertyId)
  const votes = new Map();
  for (const tx of transactions) {
    const cat = normalizeCategory(tx.category);
    // Skip: excluded, blank, or auto-assigned generic category (not intentionally set by user)
    if (tx.excluded) continue;
    if (!cat) continue;
    if (GENERIC.has(cat) && !tx.categorized) continue;
    const key = normalize(tx.description);
    if (!key) continue;
    const prev = votes.get(key) ?? { category: tx.category, propertyId: tx.propertyId || '', count: 0 };
    // Most recent/frequent category wins — increment count if same category
    const count = tx.category === prev.category ? prev.count + 1 : prev.count;
    if (count >= prev.count) votes.set(key, { category: tx.category, propertyId: tx.propertyId || '', count });
  }
  return votes;
}
...
export function isUncategorized(tx) {
  const cat = normalizeCategory(tx.category);
  if (!cat) return true;           // blank category — always uncategorized
  if (tx.categorized) return false; // user explicitly chose this category
  return GENERIC.has(cat);         // auto-assigned legacy category, not intentionally set
}
